...
class DataProcessv1(DataProcess):
    ...
    def map_process(self, traj):
        ...
        for signal in self.traffic_signals[self.hist_le n -1].lane_states:
        ...
        for data_file in self.data_files:
            ...
            for data in dataset:
                ...
                for pairs in self.sdc_ids_list:
                    ...
                    ego, neighbors, map_lanes, map_crosswalks, ground_truth, region_dict = self.normalize_data(ego,
                                                                                                               neighbors,
                                                                                                               map_lanes,
                                                                                                               map_crosswalks,
                                                                                                               ground_truth,
                    ...
                    if test:
                        np.savez(filename, ego=np.array(ego), neighbors=np.array(neighbors),
                                 map_lanes=np.array(map_lanes),
                                 map_crosswalks=np.array(map_crosswalks), object_type=np.array(object_type),
...
def main():
    ...
    data_files = glob.glob(args.load_pat h +'/*')
